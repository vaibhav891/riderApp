...
enum ReasonForRejection { damagedPoduct, changeOfMind, other }

enum ProductOrderFilter { all, scanned, nonScanned, outOfStock }

class Helper {
  BuildContext context;
  DateTime currentBackPressTime;
  static const List<String> pickerStatuses = ['1', '2', '6'];
  static const List<String> delivererStatuses = ['3', '4'];
  // static const reasonsForRejection = [
  //   'Damaged Product',
  //   'Change of Mind',
  //   'Other'
  // ];
  static const reasonForRejectionValues = {
    ReasonForRejection.damagedPoduct: 'Damaged Product',
    ReasonForRejection.changeOfMind: 'Change of mind',
    ReasonForRejection.other: 'Other',
  };

  static const productOrderFilterValues = {
    ProductOrderFilter.all: 'All',
    ProductOrderFilter.scanned: 'Picked',
    ProductOrderFilter.nonScanned: 'Non-Picked',
    ProductOrderFilter.outOfStock: 'Out of stock',
  };
  ...
  static getDataForReturnOrders(Map<String, dynamic> data) {
  ...
  static String getIntData(Map<String, dynamic> data) {
...
//        },
  ...
  static Widget getPrice(String myPrice, BuildContext context, {TextStyle style}) {
    if (style != null) {
      ...
      return RichText(
        ...
        text: setting.value?.currencyRight != null && setting.value?.currencyRight == false
            ? TextSpan(
                ...
                children: <TextSpan>[
                  TextSpan(
                      text: double.tryParse(myPrice)?.toStringAsFixed(2) ?? '',
                      style: style ?? Theme.of(context).textTheme.subtitle1),
                ],
              )
            : TextSpan(
  ...
  static String getTotalOrderPrice(ProductOrder productOrder) {
    // String total = productOrder.price;
  ...
  static String getOrderPrice(ProductOrder productOrder) {
    // String total = productOrder.price;
    // productOrder.options.forEach((option) {
    //   total += option.price != null ? option.price : 0;
    // });
  ...
  static String getTaxOrder(Order order) {
    // String total = 0;
  ...
  static String getTotalOrdersPrice(Order order) {
    // String total = order.grandtotal;
    double total = 0.0;
    order.productOrders.forEach((productOrder) {
      total += (toDouble(productOrder.price) * toDouble(productOrder.quantity));
    });
    total += toDouble(order.deliveryFee);
    total += toDouble(order.tax);
    return total.toString();
  }

  static String getSubTotalOrdersPrice(Order order) {
    // String total = order.grandtotal;
    double subTotal = 0.0;
    order.productOrders.forEach((productOrder) {
      subTotal += (toDouble(productOrder.price) * toDouble(productOrder.quantity));
    });
    // order.productOrders.forEach((productOrder) {
    //   total += getTotalOrderPrice(productOrder);
    // });
  ...
  static String stringToStringFixed(String val, int fractionDigits) {
    return double.tryParse(val)?.toStringAsFixed(fractionDigits) ?? (0.0).toStringAsFixed(fractionDigits);
  }
}
